...
class Solution:
    ...
    def findGCD(self, nums: List[int]) -> int:
        ...
        smallest = min(nums)
        largest = max(nums)
        for d in range(largest, 0, -1):
            if smallest % d == 0 and largest % d == 0:
                return d
        return 0


solution = Solution()
solution.test(solution.findGCD([2, 5, 6, 9, 10]), 2)
solution.test(solution.findGCD([7, 5, 6, 8, 3]), 1)
solution.test(solution.findGCD([3, 3]), 3)
