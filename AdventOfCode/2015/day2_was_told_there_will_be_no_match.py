...
def part_one_solution(data) -> int:
    ...
    for data_item in data.split("\n"):
        ...
        current = (
            (2 * box_length * box_height)
            + (2 * box_width * box_height)
            + (2 * box_length * box_width)
            + min(
                [
...
def part_two_solution(data) -> int:
    ...
    for data_item in data.split("\n"):
        ...
        maxval = max(sizes)
        print(sizes, maxval)
        for ribbon_size in sizes:
            if ribbon_size != maxval:
                print(ribbon_size, ribbon_size + ribbon_size)
                box_size += ribbon_size + ribbon_size
...
if DEBUG:
    daily_input = (
        pathlib.Path(
            "/Users/bl4de/playground/cs/AdventOfCode/2015/test_input.txt")
        .read_text("utf_8")
        .strip()
    )
else:
    daily_input = (
        pathlib.Path("/Users/bl4de/playground/cs/AdventOfCode/2015/input.txt")
        .read_text("utf_8")
        .strip()
    )
